...
export const useCommandPalette = ({
  ...
  useEffect(() => {
    const saved = localStorage.getItem('Viberoo_search_history');
    if (saved) {
      try {
  ...
  const handleItemSelect = (item: SearchResult) => {
    ...
    if (query.trim()) {
      ...
      localStorage.setItem('Viberoo_search_history', JSON.stringify({
        queries: newSearches,
        items: newHistory
      }));
    }
  ...
  return {
    ...
    clearHistory: () => {
      setRecentSearches([]);
      setSearchHistory([]);
      localStorage.removeItem('Viberoo_search_history');
    }
  };
};